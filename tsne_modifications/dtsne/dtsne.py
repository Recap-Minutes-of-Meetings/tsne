...
def _binary_search_perplexity_dtsne(sqdistances, desired_perplexity, verbose):
    ...
    for i in range(n_samples):
        ...
        for _l in range(n_steps):
            ...
            for j in range(n_neighbors):
                if j != i or using_neighbors:
                    P[i, j] = math.exp(-sqdistances[i, j] * beta)  # P[i, j] = p(j|i)
                    sum_Pi += P[i, j]
        ...
        if verbose and ((i + 1) % 1000 == 0 or i + 1 == n_samples):
            print(
                "[dt-SNE] Computed conditional probabilities for sample "
                "%d / %d" % (i + 1, n_samples)
            )

    if verbose:
        print("[dt-SNE] Mean sigma: %f" % np.mean(math.sqrt(n_samples / beta_sum)))
...
def _joint_probabilities_dtsne(distances, desired_perplexity, verbose):
...
def _joint_probabilities_nn_dtsne(distances, desired_perplexity, verbose):
...
def _kl_divergence_dtsne(
    ...
    **kwargs
    ...
    """dt-SNE objective function: gradient of the KL divergence
    ...
    PQd = squareform((P - Q) * dist * gamma_ij)
...
def _gradient_descent_dtsne(
    ...
    **kwargs
    ...
    learning_rate : float, default=200.0
        The learning rate for dt-SNE is usually in the range [10.0, 1000.0]. If
    ...
    for i in range(it, n_iter):
        ...
        if check_convergence:
            ...
            if error < best_error:
                ...
                if verbose >= 2:
                    print(
                        "[dt-SNE] Iteration %d: did not make any progress "
            ...
            if grad_norm <= min_grad_norm:
                if verbose >= 2:
                    print(
                        "[dt-SNE] Iteration %d: gradient norm %f. Finished."
                        % (i + 1, grad_norm)
                    )
                break
...
class DTSNE(BaseEstimator):
    ...
    >>> X_embedded = DTSNE(n_components=2).fit_transform(X)
    >>> X_embedded.shape
    (4, 2)
    """
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.method == "exact":
            ...
            if self.metric == "precomputed":
                ...
                if self.verbose:
                    print("[dt-SNE] Computing pairwise distances...")
            ...
            P, sq_sigmas_i = _joint_probabilities_dtsne(
            ...
            if self.verbose:
                print("[dt-SNE] Computing {} nearest neighbors...".format(n_neighbors))
            ...
            if self.verbose:
                print(
                    "[dt-SNE] Indexed {} samples in {:.3f}s...".format(
                        n_samples, duration
                    )
                )
            ...
            if self.verbose:
                print(
                    "[dt-SNE] Computed neighbors for {} samples "
            ...
            # compute the joint probability distribution for the input space
            P, sq_sigmas_i = _joint_probabilities_nn_dtsne(
    ...
    def _dtsne(
        ...
        """Runs dt-SNE."""
        # dt-SNE minimizes the Kullback-Leiber divergence of the Gaussians P
        ...
        sigmas_i = np.sqrt(sq_sigmas_i)
        gamma_ij = 1 / ((sigmas_i.reshape(-1, 1) + sigmas_i.reshape(1, -1)) ** 2)
        gamma_ij /= gamma_ij.max()
        gamma_ij = squareform(gamma_ij, checks=False)
        ...
        obj_func = _kl_divergence_dtsne

        # Learning schedule (part 1): do 250 iteration with lower momentum but
        # higher learning rate controlled via the early exaggeration parameter
        P *= self.early_exaggeration
        params, kl_divergence, it = _gradient_descent_dtsne(
            obj_func, params, **opt_args
        )
        if self.verbose:
            print(
                "[dt-SNE] KL divergence after %d iterations with early "
        ...
        if self.verbose:
            print(
                "[dt-SNE] KL divergence after %d iterations: %f"
                % (it + 1, kl_divergence)
            )
